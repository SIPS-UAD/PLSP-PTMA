...
const NewsCard = ({
  ...
  const detailUrl = slug ? `/${type}/${slug}` : '#';

  return (
    <Link href={detailUrl} className="block border-b-1 pb-2 sm:border-0">
      <div className="flex h-20 transform transition-transform duration-300 hover:-translate-y-0.5 sm:h-56 sm:flex-col md:h-64 xl:h-72">
        ...
        <div className="px-4 sm:px-0 sm:pt-2">
          ...
          <h3 className="line-clamp-2 text-base leading-5 font-semibold text-gray-800 transition-colors hover:text-blue-600 md:text-lg">
            {title}
          </h3>
        </div>
      </div>
